...
export default (queue) => {
  queue
    ...
    .on('job complete', (id, result) => {
      Logger.log('info', 'Job %s completed, result: %s', typeof result === 'string' ? result : JSON.stringify(result))
    })
    .on('job failed', (id, error) => {
      Logger.log('error', 'Job %s failed, error: %s', typeof error === 'string' ? error : JSON.stringify(error))
    })
    .on('job failed attempt', (id, error, attempts) => { // eslint-disable-line no-unused-vars
      Logger.log('error', 'Job %s failed (attempts: %d), error: %s', typeof error === 'string' ? error : JSON.stringify(error))
    })
    .on('job remove', (id) => {
